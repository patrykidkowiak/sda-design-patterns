...
public class CompanyWriterFactory implements Factory<String,CompanyWriter> {
    ...
    public CompanyWriter create(String path) {
        if(path.endsWith(".txt")) return new TxtCompanyWriter(path);
        else if(path.endsWith(".xml")) return new XmlCompanyWriter(path);
        else throw new IllegalArgumentException("Not supported file type");
    }
}
